...
rm %{buildroot}%{_libdir}/libmariadb.so
rm %{buildroot}%{_libdir}/mysql/plugin/{auth_gssapi_client.so,caching_sha2_password.so,client_ed25519.so,dialog.so,mysql_clear_password.so,remote_io.so,sha256_password.so}
rm %{buildroot}%{_libdir}/pkgconfig/libmariadb.pc
rm %{buildroot}%{_includedir}/mysql/{errmsg.h,ma_list.h,ma_pvio.h,ma_tls.h,mysql_version.h,mysqld_error.h,mariadb_com.h,mariadb_ctype.h,mariadb_dyncol.h,mariadb_rpl.h,mariadb_stmt.h,mariadb_version-x86_64.h,mariadb_version.h, mysql.h}
